...
# --- Configuration ---
TOKEN = os.environ.get('TOKEN', '8941363938:AAEVaQ_tsW6_1-zzjMVNjqRxERA4zwEizX8')
...
if not TOKEN:
    raise RuntimeError('Bot token set karo: TOKEN environment variable ya hardcoded value do.')
...
def _logic_updates_channel(message):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        primary_inline_button("📢 MANAS UPDATES ", url="https://t.me/Alphaa6m")   # Changed channel link
    )
...
def handle_callbacks(call):
    if call.data.startswith('verify_channel_'):
        ...
        return
    if call.data.startswith('approve_upload_') or call.data.startswith('reject_upload_'):
        return
    if not check_subscription_and_continue(None, call):
        return
    global bot_locked
...
if Flask is not None:
    ...
    @app.get('/')
    def health():
        return 'PREMIUM MANAS HOSTING BOT is running', 200

    @app.get('/health')
    def health_check():
        return {'status': 'ok', 'bot': 'PREMIUM MANAS HOSTING BOT'}, 200
...
def start_bot():
    logger.info('PREMIUM MANAS  HOSTING BOT starting...')

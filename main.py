...
packages_to_install = ['PyYAML', 'psutil', 'cryptography', 'pyTelegramBotAPI', 'flask', 'requests']
...
TOKEN = '8035097531:AAEPbE6eql25W74qs_OpgQYg1WBo9kdfVTE'
OWNER_ID = 6255440067
ADMIN_ID = 6255440067
YOUR_USERNAME = 'SGCODEX'
UPDATE_CHANNEL = 'https://t.me/SGCODEXS'
...
class SecureFileSystem:
    ...
    def _log_access(self, user_id, file_name, success):
        ...
        c = conn.cursor()
        try:
            c.execute('''INSERT INTO access_logs (user_id, file_name, access_time, success)
                        VALUES (?, ?, ?, ?)''',
    ...
    def request_approval(self, user_id, file_name):
        ...
        c = conn.cursor()
        try:
            c.execute('''INSERT INTO approval_requests (user_id, file_name, request_time, status)
                        VALUES (?, ?, ?, ?)''',
    ...
    def approve_file(self, user_id, file_name, admin_id, note=None):
        ...
        c = conn.cursor()
        try:
            c.execute('''UPDATE secure_files SET is_approved = 1, approved_by = ?, approved_at = ?
                        WHERE user_id = ? AND file_name = ?''',
                     (admin_id, datetime.now().isoformat(), user_id, file_name))
            
            c.execute('''UPDATE approval_requests SET status = 'approved', admin_id = ?, admin_note = ?, processed_at = ?
    ...
    def reject_file(self, user_id, file_name, admin_id, reason=None):
        ...
        c = conn.cursor()
        try:
            c.execute('''UPDATE approval_requests SET status = 'rejected', admin_id = ?, admin_note = ?, processed_at = ?
                        WHERE user_id = ? AND file_name = ? AND status = 'pending' ''',
    ...
    def get_user_file_names(self, user_id):
        conn = sqlite3.connect(os.path.join(self.keys_dir, 'secure_files.db'))
        c = conn.cursor()
        try:
            c.execute('SELECT file_name FROM secure_files WHERE user_id = ? AND is_deleted = 0', (user_id,))
...
def get_user_file_count(user_id):
    files = secure_fs.get_user_file_names(user_id)
    return len(files)

def is_bot_running(user_id, file_name):
    key = f"{user_id}_{file_name}"
    proc = bot_scripts.get(key)
...
def send_welcome(message):
    ...
    welcome_text = (
        f"👋 *Welcome! Secure File Host Bot Active Hai.*\n\n"
        f"Aap yahan apni bot script/files host kar sakte hain.\n"
        f"Direct file send karein (Py, JS, Zip etc.) upload karne ke liye.\n\n"
        f"📌 *Commands:* \n"
        f"• `/checkfiles` - Manage uploaded files\n"
        f"• `/start` - Restart main menu"
    )
    bot.reply_to(message, welcome_text, reply_markup=markup, parse_mode='Markdown')

@bot.message_handler(commands=['checkfiles'])
def cmd_check_files(message):
    show_user_files_list(message.chat.id, message.from_user.id)

def show_user_files_list(chat_id, user_id, message_id=None):
    files = secure_fs.get_user_file_names(user_id)
    if not files:
        msg = "⚠️ Aapne koi file upload nahi ki hai. File send karein upload karne ke liye."
        if message_id:
    ...
    for file_name in files:
        running_icon = "🟢" if is_bot_running(user_id, file_name) else "🔴"
        btn_text = f"{running_icon} {file_name}"
        markup.add(types.InlineKeyboardButton(btn_text, callback_data=f'file_{user_id}_{file_name}'))

    msg_text = "📁 *Aapki Uploaded Files:*\nFile par click karke start/stop/delete manage karein:"
    if message_id:
...
def cb_check_files(call):
    show_user_files_list(call.message.chat.id, call.from_user.id, call.message.message_id)
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('approve_') and not call.data.startswith(('approve_yes_', 'approve_no_')))
...
# ============================================
# 📂 FILE MANAGEMENT & EXECUTION CONTROLS
# ============================================
...
# ============================================
# ⏯️ RUN, STOP, RESTART & LOGS HANDLERS
# ============================================

@bot.callback_query_handler(func=lambda call: call.data.startswith(('start_', 'stop_', 'restart_', 'logs_', 'delete_')))
def process_file_actions(call):
    ...
    if action == 'start':
        if is_bot_running(user_id, file_name):
            bot.answer_callback_query(call.id, "⚠️ File pehle se hi run ho rahi hai!", show_alert=True)
            return
        
        decrypted_path = secure_fs.decrypt_file(user_id, file_name)
        if not decrypted_path:
            bot.answer_callback_query(call.id, "❌ Decryption fail ho gaya!", show_alert=True)
            return
            
        try:
            log_file = os.path.join(secure_fs.temp_dir, f"{key}.log")
            out_file = open(log_file, "a")
            
            ext = os.path.splitext(file_name)[1].lower()
            if ext == '.py':
                proc = subprocess.Popen([sys.executable, decrypted_path], stdout=out_file, stderr=out_file)
            elif ext == '.js':
                proc = subprocess.Popen(['node', decrypted_path], stdout=out_file, stderr=out_file)
            else:
                bot.answer_callback_query(call.id, "⚠️ Non-executable file type", show_alert=True)
                return
            ...
            bot.answer_callback_query(call.id, f"❌ Execution Error: {e}", show_alert=True)
        ...
        if proc and proc.poll() is None:
            ...
            bot.answer_callback_query(call.id, "⚠️ File running nahi hai!", show_alert=True)
        show_file_controls(call, user_id, file_name)
        ...
        if os.path.exists(log_file):
            with open(log_file, 'r') as f:
                logs = f.readlines()[-20:]
            log_text = "".join(logs) if logs else "Logs empty hain."
        ...
        secure_fs.delete_file_secure(user_id, file_name)
        bot.answer_callback_query(call.id, "🗑️ File deleted!")
        show_user_files_list(call.message.chat.id, user_id, call.message.message_id)
...
@bot.message_handler(commands=['start'])
def send_welcome(message):
    bot.reply_to(message, "👋 Welcome! Secure File Host Bot Active Hai.")
...
def process_file_password_setup(message, user_id, file_name, file_ext, temp_path, status_msg):
    ...
    if password.lower() == 'skip':
        password = None
        bot.edit_message_text("⏳ Saving file without password...", message.chat.id, status_msg.message_id)
    else:
        is_strong, msg = is_password_strong(password)
        if not is_strong:
            bot.edit_message_text(
            ...
            bot.register_next_step_handler(message, process_file_password_setup, 
                                          user_id, file_name, file_ext, temp_path, status_msg)
            return
        bot.edit_message_text("⏳ Saving file with password protection...", message.chat.id, status_msg.message_id)
    
    file_type = file_ext[1:]
    success, msg = secure_fs.save_file_secure(user_id, file_name, file_type, temp_path, password)
    
    if not success:
        bot.edit_message_text(f"❌ Failed to save: {msg}", message.chat.id, status_msg.message_id)
        return
...
def show_approvals(message):
    ...
    for req_id, user_id, file_name, request_time in pending:
        btn_text = f"📁 {file_name} (User: {user_id})"
        markup.add(types.InlineKeyboardButton(btn_text, callback_data=f'approve_{req_id}_{user_id}_{file_name}'))
    
    bot.reply_to(message, f"📋 *Pending Approvals: {len(pending)}*\n\nClick on a file to approve or reject.",
                 reply_markup=markup, parse_mode='Markdown')
...
def handle_approval(call):
    ...
    bot.edit_message_text(f"📁 *File: {file_name}*\n👤 User: {user_id}\n\nChoose action:",
                          call.message.chat.id, call.message.message_id,
                          reply_markup=markup, parse_mode='Markdown')
    bot.answer_callback_query(call.id)
...
def approve_file_callback(call):
    ...
    bot.edit_message_text(f"✅ File `{file_name}` approved!\nUser {user_id} has been notified.",
                          call.message.chat.id, call.message.message_id)
    bot.answer_callback_query(call.id, "✅ File approved!")
...
def reject_file_callback(call):
    ...
    secure_fs.reject_file(user_id, file_name, call.from_user.id, "Rejected by admin")
    try:
        bot.send_message(user_id, f"❌ Your file `{file_name}` has been *REJECTED*.", parse_mode='Markdown')
    except:
        pass
    
    bot.edit_message_text(f"❌ File `{file_name}` rejected.\nUser {user_id} has been notified.",
                          call.message.chat.id, call.message.message_id)
    bot.answer_callback_query(call.id, "❌ File rejected!")
...
def file_control_callback(call):
    try:
        ...
        if not secure_fs.is_file_approved(user_id, file_name):
            msg = "⚠️ File not approved yet!" if requesting_user_id in admin_ids else "⏳ File waiting for approval!"
...
def show_file_controls(call, user_id, file_name):
    ...
    if is_running:
        ...
        markup.row(types.InlineKeyboardButton("🟢 Start", callback_data=f'start_{user_id}_{file_name}'))
...
def show_password_callback(call):
    ...
    conn = sqlite3.connect(os.path.join(secure_fs.keys_dir, 'secure_files.db'))
    c = conn.cursor()
    c.execute('SELECT password_hash FROM secure_files WHERE user_id = ? AND file_name = ?', (user_id, file_name))
    result = c.fetchone()
    conn.close()
    ...
    if result and result[0]:
        ...
        bot.answer_callback_query(call.id, f"🔑 New password: {new_password}", show_alert=True)
        bot.send_message(call.message.chat.id, f"🔑 New password for `{file_name}`: `{new_password}`", parse_mode='Markdown')
    else:
        bot.answer_callback_query(call.id, "❌ No password set for this file!", show_alert=True)

# ============================================
# 🚀 SERVER & POOLING CONTROL
# ============================================
...
if __name__ == '__main__':
    ...
    flask_thread.start()
    logger.info("✅ Flask Keep-Alive Server thread started")

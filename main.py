...
# ============================================
# 📝 LOGGING CONFIGURATION (FIXED PLACEMENT)
# ============================================
...
# ============================================
# 📤 FILE UPLOAD HANDLER - WITH FULL SECURITY
# ============================================
...
def handle_file_upload(message):
    ...
    try:
        ...
        # Register next step for password
        bot.register_next_step_handler(message, process_file_password_setup, 
                                      user_id, file_name, file_ext, temp_path, status_msg)
...
def process_file_password_setup(message, user_id, file_name, file_ext, temp_path, status_msg):
    """Process password setup for file"""
    password = message.text.strip()
    ...
    try:
        bot.send_message(user_id, 
            f"📤 Your file `{file_name}` has been submitted for approval.\n"
            f"🔐 {'Password protected' if password else 'No password'}\n"
...
def handle_approval(call):
    ...
    markup.row(types.InlineKeyboardButton("🔙 Back", callback_data='back_to_admin'))
...
def show_file_controls(call, user_id, file_name):
    ...
    # Show password options (admin only)
...
def set_password_callback(call):
    """Set password for file (Admin only)"""
    if call.from_user.id not in admin_ids:
        bot.answer_callback_query(call.id, "⚠️ Admin only!", show_alert=True)
        return
...
def show_password_callback(call):
    ...
    if result and result[0]:
        # Generate a new password (since we can't decrypt the hash)
